...
public class PushService2 implements Runnable {

	private static PushService2 instance;
		...
		while (true) {
			...
			try {
				for (String key : sMap.keySet()) {
					...
					if (s.isOpen()) {
						String pattern = "yyyy-MM-dd hh:m:s";
						SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);
						String d = simpleDateFormat.format(new Date());

						// Date d = new Date(System.currentTimeMillis());
						s.getBasicRemote().sendText(
								"[{\"date\":\"" + d.toString() + "\",\"value\":" + Integer.toString(randomNum) + "}]");
					} else {
						sMap.remove(key);
					}
				}
